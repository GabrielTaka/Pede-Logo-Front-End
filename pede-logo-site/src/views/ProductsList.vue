    ...
    <v-row justify="center">
      <v-col cols="12">
        ...
        <v-card color="primary" class="white--text pa-3">
          <v-row>
            <v-col cols="10">
              <h3 class="title font-weight-light"> Produtos </h3>
...
<script>
  ...
  export default {
    ...
    computed: {
      getEstablishment() {
        return this.$store.state.selectedEstablishment
      }
    },
    ...
    methods: {
      async getProducts() {
        ...
        await securedAxiosInstance.get(`/api/v1/establishments/${this.getEstablishment.id}/products`,
        )
        .then((result) => {
          response = result
        }).catch((err) => {
